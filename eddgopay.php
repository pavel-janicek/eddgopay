...
function eddgopay_process_payment( $purchase_data ) {
  ...
  $language =strtoupper($iso_code[0]);
    ...
    if (empty($edd_options['eddgopay_choose_payment'])){
      $payer['default_payment_instrument'] = PaymentInstrument::PAYMENT_CARD;
    }    
...
function eddgopay_add_settings( $settings ) {
    ...
    array(
			'id' => 'eddgopay_choose_payment',
			'name' => '<strong> ' . __( 'Chci, aby si uživatel zvolil typ platby', 'eddgopay' ) . '</strong>',
			'desc' => __( 'Pokud toto zaškrtnete, tak se zobrazí výběr platby a uživatel si zvolí platební metodu', 'gopay' ),
			'type' => 'checkbox'
		),
		array(
...
function eddgopay_prepare_items($cart){
  ...
  foreach($cart as $cart_item){
    ...
    $gopay_item = array(
      'name' => $cart_item['name'],
      'amount' => $total * 100 * $cart_item['quantity'],
...
function eddgopay_pingback() {
  ...
  if ($response->hasSucceed()) {
        ...
        if ($response->json['state'] == PaymentStatus::PAID){
          ...
          $after_purchase_notify = (isset($edd_options['eddgopay_send_error_notification'])) AND (!empty($edd_options['eddgopay_send_error_notification']));
	        if ($after_purchase_notify){
		        $admin_to = eddgopay_get_admin_error_emails();
		        $admin_subject = edd_get_option('eddgopay_admin_error_subject', 'Platba #{payment_id} nebyla uhrazena');
		        $admin_subject = apply_filters( 'eddgopay_admin_error_subject', wp_strip_all_tags( $admin_subject ), $edd_order_id );
		        $admin_subject = edd_do_email_tags( $admin_subject, $edd_order_id );
		        $admin_message = edd_get_option( 'eddgopay_admin_error_text', edd_get_default_eddgopay_admin_error_notification_email() );
		        $admin_message = edd_do_email_tags( $admin_message, $edd_order_id);
		        EDD()->emails->send( $admin_to, $admin_subject, $admin_message );
	        }
...
function edd_get_default_eddgopay_admin_error_notification_email(){
  $default_email_body = 'Platební brána Gopay se pokusila aktualizovat objednávku ID {payment_id}'. "\n\n";
  $default_email_body .= 'Aktuální stav platby na GoPay: {stav_platby}'. "\n\n";
  $default_email_body .= __( 'Purchased by: ', 'easy-digital-downloads' ) . ' {name}' . "\n";
  $default_email_body .= __( 'Amount: ', 'easy-digital-downloads' ) . '{price}' . "\n";
  $message = edd_get_option( 'eddgopay_admin_error_text', false );
  $message = ! empty( $message ) ? $message : $default_email_body;
  return $message;
}
...
function eddgopay_email_settings($settings){
   ...
   function eddgopay_get_admin_error_emails(){
    global $edd_options;

    $emails = isset( $edd_options['eddgopay_send_error_notification'] ) && strlen( trim( $edd_options['eddgopay_admin_error_emails'] ) ) > 0 ? $edd_options['eddgopay_admin_error_emails'] : get_bloginfo( 'admin_email' );
    $emails = array_map( 'trim', explode( "\n", $emails ) );

    return apply_filters( 'edd_admin_error_emails', $emails );

   }
   
   function add_eddgopay_email_tag($payment_id){
    edd_add_email_tag( 'stav_platby', 'Navrácený stav platby z GoPay', 'eddgopay_edd_email_tag_stav_platby' );
   }
   
   add_action( 'edd_add_email_tags', 'add_eddgopay_email_tag' );
   //EDD()->email_tags->add( 'stav_platby', 'Navrácený stav platby z GoPay', 'eddgopay_edd_email_tag_stav_platby' );
